...
export default function Page() {
  ...
  async function handleAnalyze(e: React.FormEvent) {
    ...
    try {
      ...
      setError(
        err?.message ||
          "Could not analyze this salon right now. Please try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        >
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div>
                <h3 id="lead-modal-title" className="text-2xl font-bold">
                  {modalCopy.title}
                </h3>
            ...
            {leadSuccess ? (
              <div className="mt-6 rounded-2xl border border-green-200 bg-green-50 px-4 py-4 text-sm leading-6 text-green-800">
                Thanks — we’ve received your details. We’ll contact you with the
                next steps shortly.
              </div>
            ) : (
              <>
                <p className="mt-4 text-sm text-neutral-500">
                  No subscription. Just leave your email and we’ll follow up
